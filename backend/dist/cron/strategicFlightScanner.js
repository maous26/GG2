...
class StrategicFlightScanner {
     ...
     */
    async scanWithAI(tier, limitRoutes) {
        if (this.isScanning) {
        ...
        try {
            ...
            for (const route of routes) {
                try {
                    ...
                    await Route_1.Route.updateOne({ _id: route._id }, {
                        lastScan: new Date(),
     ...
     */
    async processDealWithAI(deal, route) {
        try {
            ...
            const validation = await intelligentPricingService_1.IntelligentPricingService.validatePriceIntelligently(deal.origin, deal.destination, deal.currentPrice, priceHistory, 'premium' // Segment par défaut
            );
     ...
     */
    async findUsersWithAdaptiveThresholds(deal) {
        const query = {
            $or: [
        ...
        const users = await User_1.default.find(query).limit(100);
        return users.filter(user => {
            const threshold = adaptiveThresholds[user.subscription_type] || 30;
     ...
     */
    async forceScan(tier, maxRoutes = 3) {
        if (tier) {
