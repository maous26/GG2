...
export class StrategicFlightScanner {
  ...
  private emailService: EnhancedEmailService | null = null;
  ...
  constructor(emailService?: EnhancedEmailService) {
    ...
    console.log(`   🧠 Intelligent Pricing: ACTIVE (Adaptive Thresholds 15-40%)`);
  }
  ...
  private async initializeStrategicRoutes(): Promise<void> {
    try {
      ...
      // Check if routes already exist to prevent duplicate initialization
      const existingRoutesCount = await Route.countDocuments();
      if (existingRoutesCount >= STRATEGIC_ROUTES.length) {
        console.log(`✅ Strategic routes already initialized (${existingRoutesCount} routes found)`);
        return;
      }
      ...
      // Créer les nouvelles routes stratégiques avec upsert pour éviter les duplicatas
      ...
      // Continue anyway - don't let route initialization block scanning
    }
  }

  /**
   * Initialise le système de pricing intelligent
   */
  private async initializeIntelligentPricing(): Promise<void> {
    try {
      console.log('🧠 Initializing Intelligent Pricing System...');
      
      const { IntelligentPricingService } = require('../services/intelligentPricingService');
      
      // Schedule route performance optimization daily at 3 AM
      cron.schedule('0 3 * * *', async () => {
        console.log('🎯 Running daily route performance optimization...');
        await IntelligentPricingService.optimizeRoutePerformance();
        console.log('✅ Route performance optimization completed');
      });
      
      console.log('✅ Intelligent Pricing System initialized with adaptive thresholds');
    } catch (error) {
      console.error('❌ Error initializing intelligent pricing:', error);
    }
  }
  ...
  /**
   * Envoie les alertes aux utilisateurs avec politiques bagages
   */
  private async sendAlertsToUsers(alert: any, users: any[], deal: FlightDeal): Promise<void> {
    ...
    // Préparer les alertes avec politique bagages pour l'envoi d'emails
    const alertsWithBaggage = users.map(user => ({
      ...
      returnDate: new Date(deal.returnDate || deal.departureDate),
      airline: deal.airline,
      baggagePolicy: alert.baggagePolicy || this.getDefaultBaggagePolicy(deal.airline)
    }));

    // Envoyer les emails avec politiques bagages
    if (this.emailService) {
      try {
        await this.emailService.sendPriceAlerts(alertsWithBaggage);
        console.log(`📧 Enhanced emails with baggage policies sent to ${users.length} users`);
      } catch (error) {
        console.error('Error sending enhanced emails:', error);
      }
    }

    console.log(`📬 Alert sent to ${users.length} users for ${deal.airline} ${deal.origin}-${deal.destination} at €${deal.currentPrice} (-${deal.discountPercentage}%) with baggage info`);
  }

  /**
   * Obtient une politique bagages par défaut selon la compagnie
   */
  private getDefaultBaggagePolicy(airline: string) {
