...
async function seedDatabase() {
    try {
        ...
        await Promise.all([
            ...
            Route_1.Route.deleteMany({}),
            ApiCall_1.ApiCall.deleteMany({})
        ]);
        // Create test users
        const hashedPassword = await bcrypt_1.default.hash('password123', 10);
        const adminPassword = await bcrypt_1.default.hash('GG2024Admin!', 10);
        ...
        const routes = await Route_1.Route.create([
            {
                origin: 'CDG',
                destination: 'JFK',
                tier: 1,
...
if (require.main === module) {
    mongoose_1.default.connect(process.env.MONGODB_URI || 'mongodb://admin:globegenius123@localhost:27017/globegenius?authSource=admin')
        .then(() => {
