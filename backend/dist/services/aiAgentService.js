...
class AIAgentService {
    constructor(pool, redis, gemini, openai) {
        this.pool = pool;
        this.redis = redis;
        this.gemini = gemini;
        this.openai = openai;
     ...
     */
    async optimizeRoutesIntelligently() {
        try {
            // Get current routes
            const { rows: routes } = await this.pool.query('SELECT * FROM routes');
      ...
      `);
            ...
            // Get updated routes
            const { rows: updatedRoutes } = await this.pool.query('SELECT * FROM routes');
     ...
     */
    async trackModelUsage(model, tokens) {
        try {
    ...
    async getRouteSuggestions() {
      ...
      `);
            return rows.map((r) => ({
                ...r,
                total_alerts: parseInt(r.total_alerts) || 0,
                avg_discount: parseFloat(r.avg_discount) || 0
            }));
        }
        catch (error) {
     ...
     */
    async generatePersonalizedContent(userId, type) {
        try {
            // Get user data
    ...
    buildOptimizationPrompt(routes, performance) {
        // Build prompt for AI
