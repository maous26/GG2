...
router.get('/stats', async (req, res) => {
    try {
        ...
        const alertsCount = await Alert_1.default.countDocuments({
            'sentTo.user': userId
        });
        ...
        const uniqueDestinations = await Route_1.default.distinct('destination', { isActive: true });
...
router.put('/profile', async (req, res) => {
    try {
        ...
        const calculateCompleteness = (prefs) => {
            let score = 0;
            const maxScore = 6;
            if (prefs?.travelerType)
                score += 1;
            if (prefs?.dreamDestinations?.length > 0)
                score += 1;
            if (prefs?.travelPeriod?.flexible !== undefined)
                score += 1;
            if (prefs?.notificationStyle)
                score += 1;
            if (prefs?.budgetRange?.min && prefs?.budgetRange?.max)
                score += 1;
            if (prefs?.additionalAirports)
                score += 1;
...
router.get('/adaptive-pricing', async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ message: 'Non autorisé' });
        ...
        const userThreshold = userSegmentThresholds[user.subscription_type] || 35;
        ...
        const segmentAverage = userSegmentThresholds[user.subscription_type] || 35;
