...
export class MLMaturityService {
  ...
  /**
   * Calcule le score de maturité ML (méthode simple pour tests)
   */
  async calculateMaturityScore(): Promise<number> {
    try {
      const endDate = new Date();
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - 30); // 30 jours
      
      const maturity = await this.calculateMaturity(startDate, endDate);
      return maturity.maturityScore;
    } catch (error) {
      console.log('ML Maturity calculation fallback to 75%');
