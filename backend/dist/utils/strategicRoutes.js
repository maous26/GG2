...
exports.STRATEGIC_ROUTES = void 0;
exports.getAdaptiveFrequency = getAdaptiveFrequency;
exports.calculateRouteStrategy = calculateRouteStrategy;
exports.calculateGeographicCoverage = calculateGeographicCoverage;
exports.identifyGeographicGaps = identifyGeographicGaps;
exports.getRegionPriority = getRegionPriority;
exports.getRecommendedRoutesForRegion = getRecommendedRoutesForRegion;
exports.generateAdaptiveRecommendations = generateAdaptiveRecommendations;
exports.getRoutesByTier = getRoutesByTier;
exports.getRoutesByUserType = getRoutesByUserType;
exports.getHighPriorityRoutes = getHighPriorityRoutes;
exports.getSeasonalRoutes = getSeasonalRoutes;
exports.getAiTestRoutes = getAiTestRoutes;
exports.getRoutesByRegion = getRoutesByRegion;
exports.convertToRouteModel = convertToRouteModel;
exports.STRATEGIC_ROUTES = [
...
function getAdaptiveFrequency(route, date = new Date()) {
...
function calculateRouteStrategy() {
    const tier1Routes = exports.STRATEGIC_ROUTES.filter(route => route.tier === 1);
    const tier2Routes = exports.STRATEGIC_ROUTES.filter(route => route.tier === 2);
    const tier3Routes = exports.STRATEGIC_ROUTES.filter(route => route.tier === 3);
    const seasonalRoutes = exports.STRATEGIC_ROUTES.filter(route => route.seasonalBoost);
    const aiTestRoutes = exports.STRATEGIC_ROUTES.filter(route => route.aiTestRoute);
    ...
    return {
        ...
        total: {
            routes: exports.STRATEGIC_ROUTES.length,
...
function calculateGeographicCoverage() {
    const regions = exports.STRATEGIC_ROUTES.reduce((acc, route) => {
        ...
        if (route.tier === 1)
            acc[route.geographicRegion].tier1Routes++;
        if (route.tier === 2)
            acc[route.geographicRegion].tier2Routes++;
        if (route.tier === 3)
            acc[route.geographicRegion].tier3Routes++;
        return acc;
    }, {});
...
function identifyGeographicGaps(regions) {
...
function getRecommendedRoutesForRegion(region) {
...
function generateAdaptiveRecommendations(coverage) {
    const recommendations = [];
    ...
    if (coverage.score < 80) {
        recommendations.push({
            type: 'geographic_gap',
            priority: 'high',
            message: `Couverture géographique faible (${coverage.score}%). Ajouter des routes vers: ${coverage.gaps.map((g) => g.region).join(', ')}`,
            action: 'add_routes',
            estimatedCalls: 2000
        });
    }
...
function getRoutesByTier(tier) {
    return exports.STRATEGIC_ROUTES.filter(route => route.tier === tier);
}
function getRoutesByUserType(userType) {
    return exports.STRATEGIC_ROUTES.filter(route => route.targetUserTypes.includes(userType));
}
function getHighPriorityRoutes() {
    return exports.STRATEGIC_ROUTES.filter(route => route.priority === 'high');
}
function getSeasonalRoutes() {
    return exports.STRATEGIC_ROUTES.filter(route => route.seasonalBoost);
}
function getAiTestRoutes() {
    return exports.STRATEGIC_ROUTES.filter(route => route.aiTestRoute);
}
function getRoutesByRegion(region) {
    return exports.STRATEGIC_ROUTES.filter(route => route.geographicRegion === region);
}
function convertToRouteModel(strategicRoute) {
    return {
