...
export class StrategicFlightScanner {
    ...
    try {
      ...
      for (const route of routes) {
        try {
          ...
          const deals = await this.flightAPIService.detectDeals({
            origin: route.origin,
  ...
  private async processDeal(deal: FlightDeal, strategicRoute: any): Promise<void> {
    try {
      ...
      const alert = await Alert.create({
        ...
        validationMethod: validation.adaptiveFactors.userSegment as any,
          ...
          await Route.updateOne(
            ...
            { 
              $inc: { 
                ...
                'performance.successfulAlerts': interestedUsers.length,
                'performance.totalScans': 1
              },
              $set: {
  ...
  private async findInterestedUsers(deal: FlightDeal, strategicRoute: any): Promise<any[]> {
    ...
    // Ajuster les seuils selon la qualité de la validation IA
    // Si confidence > 85%, réduire les seuils de 5%
    // Si confidence < 70%, augmenter les seuils de 5%
    let adjustedFreeMin = minDiscountForFree;
    let adjustedPremiumMin = minDiscountForPremium;
    let adjustedEnterpriseMin = minDiscountForEnterprise;

    // Si la réduction est insuffisante pour les gratuits, les exclure
    if (deal.discountPercentage < adjustedFreeMin) {
    ...
    return users.filter(user => {
      ...
      if (user.subscription_type === 'free' && deal.discountPercentage < adjustedFreeMin) {
        return false;
      }
      if (user.subscription_type === 'premium' && deal.discountPercentage < adjustedPremiumMin) {
        return false;
      }
      if (user.subscription_type === 'enterprise' && deal.discountPercentage < adjustedEnterpriseMin) {
        return false;
      }
      return true;
    });
